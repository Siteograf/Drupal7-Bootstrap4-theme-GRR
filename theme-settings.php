...
function bootstrap_gta_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  $form['tweaks']['bootstrap_gta_container'] = array(
    '#type' => 'select',
    '#title' => t('Container type'),
    '#default_value' => theme_get_setting('bootstrap_gta_container'),
  ...
  $form['tweaks']['bootstrap_gta_datetime'] = array(
    '#type' => 'checkbox',
    '#title' => t('Show "XX time ago".'),
    '#default_value' => theme_get_setting('bootstrap_gta_datetime'),
  ...
  $form['breadcrumbs']['bootstrap_gta_breadcrumb_home'] = array(
    '#type' => 'checkbox',
    '#title' => t('Show "Home" breadcrumb link'),
    '#default_value' => theme_get_setting('bootstrap_gta_breadcrumb_home'),
    '#description' => t('If your site has a module dedicated to handling breadcrumbs already, ensure this setting is enabled.'),
  );
  $form['breadcrumbs']['bootstrap_gta_breadcrumb_title'] = array(
    '#type' => 'checkbox',
    '#title' => t('Show current page title at end'),
    '#default_value' => theme_get_setting('bootstrap_gta_breadcrumb_title'),
  ...
  $form['navbar']['bootstrap_gta_navbar_position'] = array(
    '#type' => 'select',
    '#title' => t('Navbar Position'),
    '#description' => t('Select your Navbar position.'),
    '#default_value' => theme_get_setting('bootstrap_gta_navbar_position'),
    '#options' => array(
  ...
  $form['navbar']['bootstrap_gta_navbar_menu_position'] = array(
    '#type' => 'select',
    '#title' => t('Navbar Menu Position'),
    '#description' => t('Select your Navbar Menu position.'),
    '#default_value' => theme_get_setting('bootstrap_gta_navbar_menu_position'),
    '#options' => array(
      'navbar-left' => t('Left'),
  ...
  $form['navbar']['bootstrap_gta_navbar_inverse'] = array(
    '#type' => 'checkbox',
    '#title' => t('Inverse navbar style'),
    '#description' => t('Select if you want the inverse navbar style.'),
    '#default_value' => theme_get_setting('bootstrap_gta_navbar_inverse'),
  );

  drupal_add_css(drupal_get_path('theme', 'bootstrap_gta') . '/css/settings.css');
  $form['bootstrap_gta_cdn'] = array(
    '#type' => 'fieldset',
  ...
  $form['bootstrap_gta_cdn']['bootstrap_gta_cdn'] = array(
    '#type' => 'select',
    '#title' => t('BootstrapCDN version'),
    '#options' => drupal_map_assoc(array(
      '3.3.5',
    )),
    '#default_value' => theme_get_setting('bootstrap_gta_cdn'),
    '#empty_option' => t('Disabled'),
    '#empty_value' => NULL,
  );

  $form['bootstrap_gta_cdn']['bootstrap_gta_font_awesome'] = array(
    '#type' => 'select',
    '#title' => t('Font Awesome version'),
    '#options' => drupal_map_assoc(array(
      '4.4.0',
    )),
    '#default_value' => theme_get_setting('bootstrap_gta_font_awesome'),
    '#empty_option' => t('Disabled'),
    '#empty_value' => NULL,
  );
  ...
  $default_theme_details = array(
    ...
    'thumbnail' => base_path() . drupal_get_path('theme', 'bootstrap_gta') . '/preview.jpg',
  );
  ...
  $form['bootswatch']['bootstrap_gta_bootswatch'] = array(
    '#type' => 'radios',
    '#default_value' => theme_get_setting('bootstrap_gta_bootswatch'),
  ...
  if (empty($bootswatch_themes)) {
    $form['bootswatch']['bootstrap_gta_bootswatch']['#prefix'] = '<div class="alert alert-danger messages error"><strong>' . t('ERROR') . ':</strong> ' . t('Unable to reach Bootswatch API. Please ensure the server your website is hosted on is able to initiate HTTP requests.') . '</div>';
  }
}
